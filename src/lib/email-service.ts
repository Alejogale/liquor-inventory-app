...
export async function sendOrderReport({
  ...
  const content = `
    ...
    <p>Hello,</p>
    <p>Your inventory report from <strong>${organizationName}</strong> is ready for review.</p>
    
    <div class="card">
      <h3>📋 Report Summary</h3>
      <p><strong>Report Date:</strong> ${reportDate}</p>
      <p><strong>Organization:</strong> ${organizationName}</p>
      <p><strong>Total Items Tracked:</strong> ${reportData.totalItems || 0}</p>
      <p><strong>Categories:</strong> ${reportData.categories || 0}</p>
      <p><strong>Estimated Value:</strong> $${(reportData.totalValue || 0).toFixed(2)}</p>
      <p><strong>Generated By:</strong> ${reportData.generatedBy || 'System'}</p>
    </div>

    <div class="card">
      <h3>📈 Key Insights</h3>
      <p>${reportData.summary || 'Complete inventory overview including all tracked items and categories.'}</p>
      <p>This report helps you:</p>
      <ul style="text-align: left; padding-left: 20px; margin: 12px 0;">
        <li>📦 <strong>Track inventory levels</strong> across all locations</li>
        <li>💰 <strong>Monitor asset values</strong> and investment</li>
        <li>📊 <strong>Identify trends</strong> in your organization</li>
        <li>🎯 <strong>Make informed decisions</strong> about restocking</li>
      </ul>
    </div>
    ...
    <p>Questions about this report? Contact our support team at <a href="mailto:invyeasy@gmail.com" style="color: ${EMAIL_DESIGN_SYSTEM.colors.primary};">invyeasy@gmail.com</a></p>
  `
