-- SIMPLE AUTHENTICATION DIAGNOSIS
-- This script diagnoses the authentication issue without column name problems
...
-- Check if there are any data in the main tables (using basic queries)
...
-- Show sample data if any exists (using basic queries)
SELECT 'Sample categories:' as info;
SELECT id, organization_id FROM categories LIMIT 5;

SELECT 'Sample suppliers:' as info;
SELECT id, organization_id FROM suppliers LIMIT 5;

SELECT 'Sample inventory items:' as info;
SELECT id, organization_id FROM inventory_items LIMIT 5;

SELECT 'Sample rooms:' as info;
SELECT id, organization_id FROM rooms LIMIT 5;
