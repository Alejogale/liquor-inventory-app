-- =============================================================================
-- DELETE 4 SPECIFIC TEST USERS - SAFE STEP-BY-STEP GUIDE
-- =============================================================================
-- Users to delete:
...
-- =============================================================================
-- This shows you all the data for these 4 users before deletion
...
SELECT
    ...
    (SELECT COUNT(*) FROM activity_logs WHERE user_id = up.id) as activity_logs_old,
...
-- =============================================================================
-- This runs the deletion but doesn't save it - SAFE TO RUN
-- Review the output, if it looks good, proceed to STEP 3
...
BEGIN
    ...
    -- 4. Delete activity logs (if separate table exists)
    RAISE NOTICE 'Step 4: Deleting activity_logs...';
    ...
    GET DIAGNOSTICS deleted_activity_old = ROW_COUNT;
    RAISE NOTICE '  ✓ Deleted % activity logs', deleted_activity_old;

    -- 5. Delete from user_profiles
    RAISE NOTICE 'Step 5: Deleting user profiles...';
    ...
    RAISE NOTICE 'Step 6: Deleting from auth.users...';
    ...
    RAISE NOTICE 'Activity logs deleted: %', deleted_activity_old;
...
-- ROLLBACK the dry run (nothing is saved)
ROLLBACK;
...
-- Only run this after reviewing the dry run output from Step 2
...
BEGIN
    ...
    RAISE NOTICE 'Deleting activity_logs...';
    DELETE FROM activity_logs
    WHERE user_id IN (
        SELECT id FROM user_profiles WHERE email IN (
            'vikijakubikova@gmail.com',
...
-- =============================================================================
-- Run this after Step 3 to confirm deletion was successful
...
-- If both queries return 0 rows, deletion was successful! ✅
*/

-- =============================================================================
-- EMERGENCY ROLLBACK (if something goes wrong)
-- =============================================================================
-- If you're in the middle of Step 3 and something looks wrong, run:
-- ROLLBACK;
